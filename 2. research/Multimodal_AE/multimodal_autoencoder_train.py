...
import multimodal_autoencoder_model as model
import multimodal_autoencoder_dataloader as dset
...
def salt_and_pepper(img, prob):
    """salt and pepper noise for data"""
...
if options.fold is None:
    dataloader = torch.utils.data.DataLoader(
        dset.multimodal_dataloader(options.image_dataroot, options.depth_dataroot, options.semantic_dataroot, transform),
        batch_size=options.batchSize, shuffle = True, num_workers = options.workers)

unorm = visualizer_tools.UnNormalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))
...
encoder_RGB_R.apply(utils.weights_init)
...
class RMSEloss(nn.Module):
    def forward(selfself, input, targets, mask, size_average=False):
...
line_win_dict = visualize_tools.wind_dict()
