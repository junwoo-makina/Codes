...
if __name__ == "__main__":
    ...
    for iter in range(0,10):
        ...
        for set in range(0,8):
            ...
            np.savetxt(os.path.join(set_fold, 'test_iter' + str(iter + 1) + '_set' + str(set + 1) + 'label_te.txt'),
                       label_test)
            np.savetxt(os.path.join(set_fold, 'test_iter' + str(iter + 1) + '_set' + str(set + 1) + 'label_tr.txt'),
                       label_train)
            ...
            for i in range(1,10):
                ...
                with tf.Session() as sess:
                    ...
                    for epoch in range(FLAGS.max_epoch):
                        ...
                        for i in range(FLAGS.updates_per_epoch - 1):
                            ...
                            x, g = generate_train_batch(
                                trainData[i * FLAGS.batch_size:(i + 1) * FLAGS.batch_size, :FLAGS.input_size], epoch,
                                FLAGS.batch_size
                            )
                            tx = generate_test_batch(testData, 20, 130, int(32000*missing_rate))
                            _, loss_value = sess.run([train, loss],
                                                     {input_tensor: x, gt_tensor: g, te_input_tensor: tx})
                    ...
                    save_folder = os.path.join('/home/cheeze/PycharmProjects/KJW/research/electric_nose',
                                               train_foldername)
                    ...
                    np.savetxt(os.path.join(set_fold,
                                            'train_iter' + str(iter + 1) + '_set' + str(set + 1) + '_loss' + str(
                                                loss_index) + 'trm.txt'), trm)
